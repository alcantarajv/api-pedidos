...
Cada entrada registra a escolha, o motivo e a alternativa descartada. O registro foi alimentado etapa a etapa, durante o desenvolvimento, e não escrito no fim — são 84 decisões, agrupadas por tema. O README traz uma seleção com as que definem o projeto.

## Sumário

- [Escolhas de stack](#escolhas-de-stack)
- [Modelagem do domínio](#modelagem-do-domínio)
- [Idempotência: a mesma requisição, o mesmo efeito](#idempotência-a-mesma-requisição-o-mesmo-efeito)
- [Concorrência e consistência](#concorrência-e-consistência)
- [Mensageria: outbox e consumidores](#mensageria-outbox-e-consumidores)
- [Integração com o gateway de pagamento](#integração-com-o-gateway-de-pagamento)
- [Segurança](#segurança)
- [Persistência e migrations](#persistência-e-migrations)
- [Operação: expiração, limpeza e observabilidade](#operação-expiração-limpeza-e-observabilidade)
- [Estratégia de testes](#estratégia-de-testes)
- [Empacotamento, CI e deploy](#empacotamento-ci-e-deploy)
...
`PUT /api/produtos/{id}/estoque` recebe `{"estoqueDisponivel": 25}`, não `{"ajuste": "+10"}`. O delta parece mais natural para quem dá entrada em mercadoria, mas não é idempotente: reenviar depois de um timeout soma de novo. Com a quantidade final, repetir a chamada leva ao mesmo estado. É o raciocínio da `Idempotency-Key` aplicado ao caso mais simples — e a razão de estar aqui, em vez de um campo no `PUT` do produto.

### Estoque e situação têm endpoint próprio

Dar entrada em mercadoria e corrigir a descrição de um produto são operações diferentes, feitas em momentos diferentes e, com os papéis separados, possivelmente por pessoas diferentes. Se estoque fosse um campo do `PUT`, uma correção de preço montada às pressas poderia desfazer uma entrada de mercadoria por descuido no JSON. Desativar o produto também é transição de estado com significado próprio ("pare de aceitar pedidos"), não edição de atributo.
...
## Idempotência: a mesma requisição, o mesmo efeito
...
## Operação: expiração, limpeza e observabilidade
...
É uma decisão tomada bem antes de fazer falta, cobrando juros ao contrário: quando a primeira regra temporal apareceu, ela já era testável sem nenhuma ginástica.
